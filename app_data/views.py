...
from .dataview import get_dataview
...
def check_class_action(aaa=None, classobj=None, action=None):
    ...
    if action == "show":
        if "p_data_class_ro" in aaa["perms"]:
        ...
        if "p_data_class_rw" in aaa["perms"]:
...
def private(request):
    ...
    if "p_data_class_ro" not in aaa["perms"]:
...
def instance_list(request, classname=None):
    ...
    context = start_view(request, app="data", view="instance_list", noauth="app_sirene:index", 
        perm="p_data_instance_ro", noauthz="app_data:private")
    if context["redirect"]:
        return redirect(context["redirect"])
    aaa = context["aaa"]
    ...
    if beta_preview == "yes":
        ...
        #print("dataview_selector ",dv_selector)

        dataview_name = request.GET.get("dv",None)
        if dataview_name:
            #print("** dataview_name: ", dataview_name)
            dataview_content = get_dataview(dataview_name)
            if dataview_content:
                #print("dataview_content: ", dataview_content)

                # prepare all attributes for template
                context["dataview"] = True
                #dataview_heads = ["ville", "code_postal","departement","sirene_group"]
                dataview_heads = dataview_content["columns"]
                context["dataview_heads"] = dataview_heads
                schema = get_schema(classname=classname)
                
                for iobj in instances:
                    dataview = []
                    i = Instance(iobj=iobj, classname=classname, schema=schema)

                    for head in dataview_heads:
                        try:
                            field = i.fields[head].get_datapoint_ui_detail()
                            dataview.append(field["value"])
                        except:
                            dataview.append("")
                    # {'fieldname': 'ville', 'displayname': 'Ville', 'description': '', 'dataformat': 'string', 
                    #   'dataformat_ext': None, 'is_multi': False, 'bigset': False, 'value': ''}
                        #dataview.append(field["value"])

                    iobj.dataview = dataview
...
def instance_edit(request, classname=None, keyname=None):
    ...
    context = start_view(request, app="data", view="instance_edit", noauth="app_sirene:index", 
        perm="p_data_instance_rw", noauthz="app_data:private")
    if context["redirect"]:
        return redirect(context["redirect"])
    aaa = context["aaa"]
