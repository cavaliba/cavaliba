...
def get_instances_raw_json(classname = None, is_enabled=True, fieldname = None):
    ...
    dataclass = get_class_by_name(classname)
    if is_enabled:
        instances = DataInstance.objects.filter(classobj=dataclass).filter(is_enabled=is_enabled)
    else:
        instances = DataInstance.objects.filter(classobj=dataclass)
...
#     classdata k/v
#
# _static:staticlistname:
#  (...)



def load_data(filedata, verbose=True):

    line_total = 0
    line_ok = 0

    # _schema:{classname => 000classdata} 
    if "_schema" in filedata:
        for classname, classdata in filedata["_schema"].items():
            # class entry
            classobj = load_class_schema(classname, classdata, verbose=verbose)
            if not classobj:
                continue
            # fields
            for fieldname, fielddata in classdata.items():
                # class meta data
                #if fieldname in ["_action", "_displayname", "_is_enabled", "_icon", "_page", "_order"]:    
                if fieldname.startswith("_"):
                    continue
                fieldobj = load_field_schema(classobj, fieldname, fielddata, verbose=verbose)
                # if fieldobj:
                #     print("  field:", fieldname)

    # _schema:classname: singleton(s)
    ...
    # instances (1) : _data:  >>> classname: >>> instance: >>> attribs
    if "_data" in filedata:

        # loop over classes
        for classname, instances in filedata["_data"].items():
            classobj = get_class_by_name(classname)
            if not classobj:
                if verbose:
            ...
            # loop over instances
            for keyname, instdata in instances.items():
                r, action = load_instance(classobj=classobj, keyname=keyname, instdata=instdata, schema=schema, verbose=verbose)
                if r:
                    if verbose:
                        print(f"  data {action} - {classname} => {keyname}")



    # instances (2) singletons  : classname:instance: >> attribs
    ...
    # _static:staticname: singleton(s)
    for item, staticdata in filedata.items():
        if item.startswith("_static:"):
            staticname = re.sub("^_static:", '', item)
            if staticname == "":
                continue
            staticobj = load_static(staticname, staticdata, verbose=verbose)



def load_instance(classobj=None, keyname=None, instdata=None, schema=None, verbose=True):
    ...
    # action requested
    action = instdata.get("_action", "create_or_update")
...
def load_class_schema(classname, classdata, verbose=True):
    ...
    # meta
    action = classdata.get("_action", "create_or_update")
    if verbose:
        print(f"CLASS : {classname} ({action})")
    ...
    if action == "delete":
        ...
        if not classobj:
            ...
            classobj.is_enabled = classdata.get("_is_enabled", True) in TRUE_LIST
            classobj.save()
    ...
    if "_order" in classdata:
        classobj.order = classdata.get("_order")
...
def load_field_schema(classobj, fieldname, fielddata, verbose=True):
    ...
    # meta
    action = fielddata.get("_action", "create_or_update")
    fieldobj = get_field_by_name(classobj, fieldname)
    if fieldobj:
        if verbose:
            print(f"  field: {fieldname} ({action})")
    ...
    #         return
        ...
        # unknown action
        if verbose:
            print(f"    !!! unknown field action {action}")
        return
...
# load static lists
def load_static(staticname, staticdata, verbose=True):

    if not staticname:
        return

    if not len(staticname)>0: 
        return

    if not staticdata:
        return

    if not type(staticdata) is dict:
        return

    if not len(staticdata)>0: 
        return


    # meta
    action = staticdata.get("_action", "create_or_update")
    if verbose:
        print(f"STATIC : {staticname} ({action})")


    staticobj = DataStatic.objects.filter(keyname=staticname).first()


    if action == "disable":
        if staticobj:
            staticobj.is_enabled = False
            staticobj.save()
            return

    elif action == "enable":
        if staticobj:
            staticobj.is_enabled = True
            staticobj.save()
            return


    # delete and recreate ; no partial update / split config
    if staticobj:
        staticobj.delete()
    ...
    staticobj=DataStatic()
    staticobj.keyname = staticname
    staticobj.displayname = staticdata.get("displayname", staticname)
    staticobj.dataformat = staticdata.get("dataformat", "string")
    staticobj.is_enabled = staticdata.get("is_enabled", True) in TRUE_LIST
    staticobj.save()
    ...
    if "values" not in staticdata:
        if verbose:
            print(f"  !!! no values for static {staticname}")
        return

    values = staticdata.get("values")
    ...
    for item in values:
        try:
            ...
            if verbose:
                print(f"  !!! invalid value for static {staticname}")
            continue
        ...
        try:
            e = item["is_enabled"] in TRUE_LIST
        except:
            e= True
        if verbose:
            print("  ", o, v)

        valobj = DataStaticValue()
        valobj.datastatic = staticobj
        valobj.value = v
        valobj.order = o
        valobj.is_enabled = e
        valobj.save()
