...
from app_home.load import load_file_yaml_to_data
from app_home.load import load_file_json_to_data
from app_home.load import load_file_csv_to_data

from app_home.load import load_dict
from app_home.load import apply_pipeline
from app_home.load import get_pipeline
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--pipeline', type=str, help="pipeline name to import and preprocess data")
        parser.add_argument('--classname', type=str, help="classname for CSV file only")
        ...
        parser.add_argument('--pass', type=int, default=1, help="default 1 , number of load pass for relationships between objects")
    ...
    def handle(self, *args, **options):
        ...
        if force_action:
            if force_action not in ["create", "create_or_update","update", "delete", "enable", "disable"]:
        ...
        for fileentry in options['filenames']:
            if os.path.isfile(fileentry):
                ...
                print(f"ERR - Unknown filename (skipping): {fileentry}")
        ...
        for passcurrent in range(1,passcount+1):
            ...
            for filename in ordered:
                ...
                if filename.endswith('.csv'):
                    ...
                    print(f"ERR - Unknown filtetype ({filename}), skipping.")
                    continue
                ...
                print(f"loaded from file:  ", len(data))
