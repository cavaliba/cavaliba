...
def list(request):
    ...
    log(DEBUG, aaa=aaa, app="sirene", view="category", action="list", status="OK", data=f"")
...
def edit(request, cid=None):
    ...
    if request.method == "POST":
        ...
        if not 'p_sirene_cat_cud' in aaa["perms"]:
            messages.add_message(request, messages.WARNING, _("Not allowed"))
            log(WARNING, aaa=aaa, app="sirene", view="category", action="edit", status="KO", data=f"not allowed")
            return redirect("app_sirene:category_list")
        ...
        if form.is_valid():
            ...
            log(WARNING, aaa=aaa, app="sirene", view="category", action="edit", status="KO", data=f"Invalid form")
    # GET
    else:
        if cid:
            category = Category.objects.get(pk=cid)
            ...
            form = CategoryForm()
            log(INFO, aaa=aaa, app="sirene", view="category", action="edit", status="OK", data=f"new")
