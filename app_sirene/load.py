...
def load_sirene(data):
    ...
    if "_sirene_category" in data:
            for item in data["_sirene_category"]:
                load_category(item)

    if "_sirene_public" in data:
            for item in data["_sirene_public"]:
                load_public(item)

    if "_sirene_template" in data:
            for item in data["_sirene_template"]:
                load_template(item)


    # singleton format :  
    # "_sirene:template_name"
    # "_sirene_public:publicpage_name"
...
def load_template(data, load_pass=0, verbose = False):
    ''' Load a json structure  into database '''
