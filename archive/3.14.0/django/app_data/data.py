# (c) cavaliba.com - app_data - data.py
...
from app_home.cache import cache_classid
from app_home.cache import cache_schema
from app_home.cache import cache_instance_name
...
def get_class_by_id(id):
    ...
    if not id in cache_classid:
        obj = DataClass.objects.filter(pk=id).first()
        if obj:
            cache_classid[id] = obj
            return obj
    else:
        return cache_classid[id]
...
def get_schema(cid=None, iid=None, obj=None, classname=None, iname=None):
    ...
    global cache_schema
    if classobj.keyname in cache_schema:
        return cache_schema[classobj.keyname]
    ...
    cache_schema[classobj.keyname] = copy.deepcopy(reply_dict)
    return reply_dict
...
class Instance:
    ...
    def __init__(self, 
        ...
        # get all external fields values
        self.retrieve_external_values()
    ...
    def get_attribute(self, fieldname):
        ...
        # TODO: permissions p_ read/update/delete
    ...
    # External - recurse to access Origin value at read time ; no cache
    def retrieve_external_values(self):
        for fieldname, fieldschema in self.schema.items():           
            if fieldname.startswith('__'):
                continue   
