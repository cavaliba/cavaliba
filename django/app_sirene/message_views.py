...
def editor2(request, template_id=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(request, messages.ERROR, _("Invalid form"))
            log(INFO, aaa=aaa, app="sirene", view="message", action="edit", status="KO", data=f"Invalid form")
    ...
    # GET
        ...
        if template_id:
            ...
            form = MessageForm()
            log(DEBUG, aaa=aaa, app="sirene", view="message", action="edit", status="OK", data=f"")
