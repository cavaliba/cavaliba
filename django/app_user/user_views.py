...
def list(request):
    ...
    if output != "":
        ...
        if count > export_max:
            log(ERROR, aaa=aaa, app="user", view="list", action="export", status="KO", data="Export too big") 
            messages.add_message(request, messages.ERROR, _("Export too large for interactive export."))
            return redirect("app_user:user_list")

        if output == "csv":
            log(DEBUG, aaa=aaa, app="user", view="user_list", action="export_csv", status="OK", data="") 
            response = user_csv_response(users)
            return response   

        elif output == "json":
            log(DEBUG, aaa=aaa, app="user", view="user_list", action="export_json", status="OK", data="") 
            response = user_json_response(users)
            return response   

        elif output == "yaml":
            log(DEBUG, aaa=aaa, app="user", view="user_list", action="export_yaml", status="OK", data="") 
...
def delete(request, userid):
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            log(ERROR, aaa=aaa, app="user", view="user_delete", action="delete", status="FAIL", data="") 
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user:
                ...
                log(WARNING, aaa=aaa, app="user", view="edit", action="save", status="KO", data="")
            
            return redirect("app_user:user_detail", user.login)

        else:
            messages.add_message(request, messages.ERROR, _("Invalid User form"))
            log(DEBUG, aaa=aaa, app="user", view="edit", action="save", status="KO", data="Invalid User form")
        ...
        if user:
            ...
            log(DEBUG, aaa=aaa, app="user", view="edit", action="new", status="OK", data="")
