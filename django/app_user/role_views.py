...
def list(request):
    ...
    if output != "":
        ...
        if count > export_max:
            log(ERROR, aaa=aaa, app="user", view="role", action="export", status="KO", data="Export too big") 
...
def role_import(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            #     line_total, line_ok = group_import_csv(file)
                ...
                log(ERROR, aaa=aaa, app="user", view="role_import", action="POST", status="FAIL", data="Unknown file type")
                return redirect("app_user:role_list")          
    ...
    log(ERROR, aaa=aaa, app="user", view="role_import", action="POST", status="FAIL", data="import failed, not a POST")
    return redirect("app_user:role_list")
