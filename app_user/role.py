...
def role_import_dictlist(rows):
    ...
    for rowdict in rows:
        ...
        role, action = role_import(rowdict)


        #action = rowdict.get("action", "create_or_update")
        
        # if action == "create":
        #     role = role_create(rowdict)

        # elif action == "update":
        #     role = role_update_by_data(rowdict)

        # elif action == "delete":
        #     role = role_delete_by_data(rowdict)

        # else:
        #     role = role_create(rowdict)

        if role:
            print(f"  role {action} - {role.keyname}")            
            line_ok += 1
