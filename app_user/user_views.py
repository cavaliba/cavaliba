...
def private(request):
    ...
    context = start_view(request, app="user", view="private", noauth="app_home:private", 
        perm="p_user_access", noauthz="app_home:index")
    if context["redirect"]:
        return redirect(context["redirect"])
    aaa = context["aaa"]

    return redirect("app_user:user_list")
    # return render(request, 'app_user/private.html', context)
...
def list(request):
    ...
    context = start_view(request, app="user", view="user.edit", noauth="app_home:index", 
        perm="p_user_view", noauthz="app_sirene:index")
    if context["redirect"]:
        return redirect(context["redirect"])
    aaa = context["aaa"]
    ...
    # else:
    #     bigset = False
    # NEW: always bigset (provides filtering, paginate, ...)
    bigset = True
    ...
    if output != "":
        ...
        # permission
        if not 'p_user_export' in aaa["perms"]:
            log(WARNING, aaa=aaa, app="user", view="list", action="export", status="FAIL", data=_("Not allowed")) 
        ...
        if count > export_max:
            log(ERROR, aaa=aaa, app="user", view="list", action="export", status="KO", data=f"Export too big") 
        ...
        if output == "csv":
            ...
            log(DEBUG, aaa=aaa, app="user", view="user_list", action="export_json", status="OK", data=f"") 
            response = user_json_response(users)
            return response   

        elif output == "yaml":
            log(DEBUG, aaa=aaa, app="user", view="user_list", action="export_yaml", status="OK", data=f"") 
    ...
    if bigset:
        ...
        #print(f"*** page={page}, offset={offset}, limit={limit}, count={count} size={size}, page_last={page_last}" )
    ...
    log(DEBUG, aaa=aaa, app="user", view="list", action="get", status="OK", data=f"{count} users")
...
def delete(request, userid):
    ...
    context = start_view(request, app="user", view="user_delete", noauth="app_home:index", 
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            log(INFO, aaa=aaa, app="user", view="user_delete", action="delete", status="OK", data=f"{user.login}") 
        else:
            messages.add_message(request, messages.ERROR, _("Failed to delete user"))
            log(ERROR, aaa=aaa, app="user", view="user_delete", action="delete", status="FAIL", data=f"") 
...
def detail(request, userid=None, login=None):
    ...
    context = start_view(request, app="user", view="user_detail", 
    ...
    if not user:
        messages.add_message(request, messages.ERROR, _("Not allowed"))
        log(WARNING, aaa=aaa, app="user", view="user", action="detail", status="KO", data=_("Not allowed") )
        return redirect("app_user:user_list")
...
def edit(request, userid=None, login=None):
    ...
    context = start_view(request, app="user", view="user.edit", noauth="app_home:index", 
        perm="p_user_cud", noauthz="app_user:private")
    if context["redirect"]:
        return redirect(context["redirect"])
    aaa = context["aaa"]
    ...
    if userid:
        ...
        if not user:
            messages.add_message(request, messages.ERROR, _("Not allowed"))
            log(WARNING, aaa=aaa, app="user", view="user", action="edit", status="KO", data=_("Not allowed"))
            return redirect("app_user:user_list")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user:
                ...
                log(INFO, aaa=aaa, app="user", view="edit", action="save", status="OK", data=f"{user.login}")
            else:
                messages.add_message(request, messages.ERROR, _("Failed to save user"))
                log(WARNING, aaa=aaa, app="user", view="edit", action="save", status="KO", data=f"")
            
            return redirect("app_user:user_detail", user.login)

        else:
            messages.add_message(request, messages.ERROR, _("Invalid User form"))
            log(DEBUG, aaa=aaa, app="user", view="edit", action="save", status="KO", data=f"Invalid User form")

    else:

        if user:
            form = user_get_form(user)
            log(DEBUG, aaa=aaa, app="user", view="edit", action="edit", status="OK", data=f"{user.login}")

        else: 
            form = user_get_form_blank()
            log(DEBUG, aaa=aaa, app="user", view="edit", action="new", status="OK", data=f"")
...
def user_import(request):
    ...
    context = start_view(request, app="user", view="user_import", noauth="app_home:index", perm="p_user_import", noauthz="app_user:private")
    if context["redirect"]:
        return redirect(context["redirect"])
    aaa = get_aaa(request)
    ...
    log(ERROR, aaa=aaa, app="user", view="user_import", action="POST", status="FAIL", data=f"import failed, not a POST")
    return redirect("app_user:user_list")
...
def preferences(request):
    ...
    context = start_view(request, app="user", view="user_preferences", 
...
def logout(request):
    ...
    context = start_view(request, app="user", view="logout", noauth="app_home:index")
    if context["redirect"]:
        return redirect(context["redirect"])
    aaa = context["aaa"]

    # request.session["aaa"] = None
    # request.session.pop('aaa', None)
...
def debug_env(request):
    ...
    context = start_view(request, app="sirene", view="debug_env", 
    ...
    try:
        ...
        tokid_header  = json.dumps(toki_header, indent=4)
        tokid_payload = json.dumps(toki_payload, indent=4)
    except:
        tokid_header = "n/a"
        tokid_payload = "n/a"
...
def email_test(request, userid=None):
    ...
    context = start_view(request, app="user", view="email_test", 
...
def sms_test(request, userid=None):
    ...
    context = start_view(request, app="user", view="sms_test", 
