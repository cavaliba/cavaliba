...
from .models import SireneAPIKey
...
def api_update_error(keyname=None):
    ...
    if not keyname:
        return
    SireneAPIKey.objects.filter(keyname=keyname).update(error_count=F("error_count") + 1, last_error = timezone.now())
...
def api_update_success(keyname=None):
    ...
    if not keyname:
        return
    SireneAPIKey.objects.filter(keyname=keyname).update(success_count=F("success_count") + 1, last_success = timezone.now())
...
def start_api(request):
    ...
    sirene_auth = request.headers.get('X-Sirene-Key','')
    if not sirene_auth:
        reply["error"] = "api - no key in header"
        return reply

    (keyname, keyvalue) = sirene_auth.split( )
    ...
    # DB
    keyobj = SireneAPIKey.objects.filter(keyname = keyname).prefetch_related("permissions").first()
    if not keyobj:
        reply["error"] = f"api - non-existent key {keyname} in DB"
        return reply

    # is_enabled
    if not keyobj.is_enabled:
        reply["error"] = f"api - key {keyname} disabled"
        api_update_error(keyname)
        return reply

    # keyvalue
    if not keyobj.keyvalue == keyvalue:
        reply["error"] = f"api - invalid keyvalue for key {keyname}"
        api_update_error(keyname)
        return reply

    # is_readonly
    reply["is_readonly"] = keyobj.is_readonly
    ...
    # get ACL/objects
    reply["acl_filter"] = keyobj.acl_filter

    # get permissions 
    for i in keyobj.permissions.all():
        reply["permissions"].append(i.keyname)
...
def start_view(request, app="na", view="na", noauth=None, perm=None, noauthz=None, visitor=False):
    ...
    if debug_aaa == "yes":
        print("------------------------------------")
        print("START VIEW - started: ", app, view)
    ...
    if  debug_aaa2 == "yes":
        ...
        print("  is_sysadmin:", aaa["is_sysadmin"] )
    ...
    if perm:
        if not perm in aaa["perms"]:
            #messages.add_message(request, messages.ERROR, _("Not allowed.") )
            log(WARNING, aaa=aaa, app=app, view=view, action="access", data=f"Not allowed")
            context["redirect"] = noauthz
            #print(context)
            return context

    #log(DEBUG, aaa=aaa, app=app, view=view, action="access", status="OK", data="start view")
...
# is_authenticated   : authenticated & exist in SireneUser DB (or sysadmin)
#                     - auth basic (HTTP header)
#                     - fed/oauth2
#                     - sirene internal auth forms [TBD]
#                     - session from previous
# is_sysadmin        : built-in sysadmin account
# user               : dict serialized SireneUser
# username           : login
# userid             : pk from DB
# user_ip            : 
# auth_mode          : oauth2, basic, sirene, forced, unittest (hidden)
# perms:[]           : [keyname, ...]
...
def get_aaa(request):
    ...
    # user not in DB
        ...
        if debug_aaa == "yes":
             print(f"DEBUG_AAA - user NOT in DB , assuming visitor for now")
        ...
        if uname == "sysadmin":
            ...
            if debug_aaa == "yes":
                print("DEBUG_AAA - uname is sysadmin")
    ...
    # built-in user sysadmin ?
    if aaa["username"] == "sysadmin":
        aaa["is_sysadmin"] = True
        if debug_aaa == "yes":
            print("DEBUG_AAA - username is sysadmin => is_sysadmin=True")

    # built-in role_sysadmin group ?
    if "role_sysadmin" in groups + groups_indirect:
        aaa["is_sysadmin"] = True
        if debug_aaa == "yes":
            print("DEBUG_AAA - role_sysadmin => is_sysadmin=True")
    ...
    if aaa["is_sysadmin"]:
        ...
        if debug_aaa == "yes":
            print("DEBUG_AAA - all groups, perms for sysadmin")
