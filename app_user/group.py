...
#from .common import make_dictlist_from_csv
...
def group_import_dictlist(rows):
    ...
    for rowdict in rows:
        ...
        group, action = group_import(rowdict)


        # action = rowdict.get("action", "create_or_update")
        
        # if action == "create":
        #     group = group_create(rowdict)

        # elif action == "update":
        #     group = group_update_by_data(rowdict)

        # elif action == "delete":
        #     group = group_delete_by_data(rowdict)

        # else:
        #     group = group_create(rowdict)

        if group:
            print(f"  group {action} - {group.keyname}")            
            line_ok += 1
